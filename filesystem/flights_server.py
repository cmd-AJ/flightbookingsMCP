...
def search_flights(
    ...
    try:
        ...
        if origin:
            query["From"] = {"$regex": origin, "$options": "i"}
        if destination:
...
@mcp.tool()
def search_by_route(origin: str, destination: str, limit: int = 20) -> str:
    """Search flights for a specific route"""
    try:
        collection = get_collection()
        
        query = {
            "From": {"$regex": origin, "$options": "i"},
        ...
        return f"Flights from {origin} to {destination} (sorted by price):\n{json.dumps(results, indent=2)}"
